...
const Weave = ( { location } ) => {    
    ...
    const totalTime = 20*8 // OR, 180 s === 3 minutes
    ...
    const changeProse = (event) => {    
        setSpanArray(((prev) => prev.map((val) => val.keyPropId === index? {...val, userProse: event.target.value, randomColor: hexAlpha(randomColor, 0.1) } : val )))
    ...
    useEffect(() => {
        ...
        socket.on('friendProse', ({ friendProse, randomColor }) => {                       
            setSpanArray(((prev) => prev.map((val) => val.keyPropId === index? {...val, userProse: friendProse, randomColor: hexAlpha(randomColor, 0.16)} : val )))
        })
        ...
        <div className="weave-form" id="weave-id">
            <Container>
                <Paper>  
                    ...
                    <div className="weave-story">{ spanArray.map((val) => <span id="span-writer" key={val.keyPropId} style={{ backgroundColor: val.randomColor }}>{val.userProse} </span>) }</div>
